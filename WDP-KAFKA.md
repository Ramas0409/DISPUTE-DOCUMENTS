...
| Authentication | AWS IAM (MSK IAM auth — `SASL_SSL` + `AWS_MSK_IAM` mechanism + `IAMLoginModule` + `IAMClientCallbackHandler`) | Confirmed uniformly across COMP-04, COMP-05, COMP-14, COMP-15, COMP-16, COMP-17, COMP-18, COMP-39, COMP-40, COMP-41, COMP-42, COMP-43 |
...
### Hidden coupling — retry config (COMP-04)

> ⚠️ **`${kafka_retry_count}` and `${kafka_retry_delay}` on COMP-04 NAPDisputeEventService**
> govern not only the Kafka publish to `nap-dispute-events`, but also the
> CaseManagement (new-event path), FraudSwitch, and DisplayCodeService
> outbound REST calls. Tuning Kafka retries silently retunes those three
> REST dependencies. Operators tuning Kafka behaviour need to be aware of
> the cross-channel side effects.

### Empty `CommonErrorHandler` — platform pattern (deserialization silent-drop)

> ⚠️ The empty anonymous `CommonErrorHandler{}` factory wiring is a
> platform-wide pattern. On bad payload, deserialization exceptions are
> silently swallowed — no DLT, no audit row, no metric, often no log.
> **Confirmed components with this pattern:** COMP-05, COMP-14, COMP-16,
> COMP-18, COMP-40, COMP-41, COMP-42, COMP-43. COMP-15 and COMP-17 differ
> slightly (see component notes). RISK candidate already raised against
> RISK-018 / RISK-040.

### Operational footgun — env vars without defaults

> ⚠️ Multiple components fail to start if certain env vars are unset
> because no `application.yaml` default is provided. Confirmed:
> `${kafka_business_event_topic}` on COMP-25 NotesService;
> `${kafka_retry_count}` on COMP-04 NAPDisputeEventService.
> This is platform-pattern build hygiene — same severity class.
...
|-----------|-------|-----------------|
| COMP-05 NAPDisputeEventProcessor | `nap-dispute-events` | Pre-ACK — first call in `onMessage()`, before the case service invocation. ✅ Source-verified. |
| COMP-12 InboundDisputeEventScheduler | (producer only — outbox mark-before-send) | Mark-before-send within a single `@Transactional` — broker ACK precedes TX commit. **At-least-once with duplicate-possible crash window**. Consumer-side `idempotency-key` dedup is the intended mitigation. |
...
| COMP-16 BusinessRulesProcessor | `business-rules` | **Two-branch consumer (v1.3, 2026-05-15).** Pre-ACK on BOTH consumer branches — branch is selected on `eventId` null vs non-null. **Branch A (eventId non-null):** ack precedes `processRulesEvent`. **Branch B (eventId null):** `processNewCaseActionEvent` persist → ACK → `processRulesEvent`. Both branches at-most-once. The Branch B persist writes a `WDP.bre_orchestration_outbox` BUSINESS_RULES row before ACK, which carries application-level dedup (see DEC-001 PARTIAL row below). Pre-`processRulesEvent` ACK gap unchanged. |
...
| COMP-39 NAPOutcomeProcessor | `internal-integration-events` | Pre-ACK — `acknowledgment.acknowledge()` as first call in `onMessage()`. ✅ Re-confirmed. Concurrency=1 (Spring default — `setConcurrency()` not invoked). |
| COMP-40 VisaResponseQuestionnaire | `internal-integration-events` | Pre-ACK — `acknowledge()` at `KafkaConsumer.java:L30` precedes `processContestEvent()` at `:L32`. ✅ Re-confirmed. |
...
| COMP-42 BENConsumer | `external-request-events` | Pre-ACK after idempotency DB check (Step 3); before CMS call (Step 6), BEN Product call (Step 7), CAS call (Step 10), BEN Kafka publish (Step 12), outbox status update (Step 13). ✅ Confirmed from COMP-42 source. |
...
| Component | Topic | Deviation detail |
|-----------|-------|-----------------|
| COMP-04 NAPDisputeEventService | `nap-dispute-events` | Direct synchronous publish on the inbound HTTP thread — `kafkaTemplate.send().get()` with no timeout, `@Retryable` retry. No outbox. No at-least-once delivery guarantee. Events lost between enrichment completion and broker ACK are unrecoverable. ✅ Source-verified. |
...
| COMP-16 BusinessRulesProcessor | `outgoing-events`, `internal-integration-events` | ⚠️ **PARTIAL (v1.3, 2026-05-15)** — `WDP.bre_orchestration_outbox` now exists as event-level status register. Statuses observed: PUBLISHED, SUCCESS, FAILED, ERROR, PENDING_DEFERRED, SKIPPED. **Kafka publishes still direct synchronous (no relay)** — broker unavailable still produces a FAILED row but no automatic re-drive (Scheduler4 reads only FAILED/PENDING_DEFERRED). **Cross-datasource non-atomic UK write window:** UK audit log (`nap.br_case_audit_log`, `ukDataSource`) and outbox (`WDP.bre_orchestration_outbox`, `wdpDataSource`) are not in the same transaction and cannot be — different datasources, no XA, no saga. See RISK-203 and ADR-CAND-070. US path is single-datasource and `@Transactional`-bracketed. |
...
| COMP-23 CaseManagementService | `business-rules` | Synchronous publish **inside `@Transactional` before commit** — `kafka-before-commit` pattern. Send failure rolls back the DB; broker ACK followed by a later commit-time exception emits a published-but-unpersisted event (narrow but non-zero window). |
| COMP-24 CaseActionService | `business-rules`, ActionEvent topic | ⚠️ PARTIAL — `wdp.chbk_outbox_row` / `nap.DISPUTE_EVENT_CONSUMER_ERROR` inbound-outbox UPDATEs are inside the same `@Transactional` as the domain write. **BRE publish is also inside `@Transactional`**. **ActionEvent publish is outside `@Transactional` on EP 2 / 8 / 9 — genuine post-commit split-brain when `napUpdateEvent=true`.** |
| COMP-25 NotesService | `business-rules` | Synchronous publish inside `@Transactional` boundary — not atomically coupled. ⚠️ **Mid-batch atomicity note:** the per-event loop runs inside `@Transactional`, so partial-batch failure produces deterministic Kafka-orphan events — the failure mode is not just JVM-crash but normal partial-batch failure. |
...
|-----------|-------|-----------|------|
| COMP-04 NAPDisputeEventService | `nap-dispute-events` | **Per-endpoint:** `merchantId` on POST `/{caseId}/{uniqueId}` (case update) and POST `/outcome/...` (win/loss outcome); `cardAcceptorCodeId` on POST `/event` (new disputes only — sourced from `transactions[0].meta.card_acceptor_code_id`). | Partial deviation — decommission-scoped. Idempotent producer enabled (`ENABLE_IDEMPOTENCE_CONFIG=true`, `MAX_IN_FLIGHT=1`). |
...
| COMP-14 CaseCreationConsumer | `new-case-events` | (Consumer side) compound key logged as `keyNetworkCaseCardNetworkId` — not used for routing inside the consumer. Per-partition ordering is scoped to the compound key, not `merchantId`. | Consumer-side confirmed — deviation confirmed both sides |
...
| COMP-24 CaseActionService | `business-rules`, ActionEvent topic | `caseNumber` | Confirmed — consistent with platform-wide deviation pattern across all six business-rules publishers |
...
| COMP-42 BENConsumer | `external-request-events` (consumer side) | Inbound key mapped to local variable `caseNumber` of type String via `@Header(KafkaHeaders.RECEIVED_KEY)` — strongly implies key is `caseNumber`, not `merchantId`. **Requires confirmation from COMP-18 producer source.** | Consumer-side observation re-confirmed |
...
### Confirmed `auto.offset.reset = latest` consumers

> Cold-start with no committed offset **skips backlog** rather than replaying. Confirmed across:
> COMP-05, COMP-14, COMP-15, COMP-16, COMP-17, COMP-18, COMP-39, COMP-40, COMP-41, COMP-42, COMP-43.
> Operational implication: a consumer-group rename or reset under load will silently drop any
> messages produced before the new group joins.
...
|------------|-----------|-------------|---------------|--------|----------------|
| `nap-dispute-events` | COMP-04 NAPDisputeEventService — confirmed source-verified; idempotent producer (`ENABLE_IDEMPOTENCE_CONFIG=true`, `MAX_IN_FLIGHT=1`); synchronous publish on inbound HTTP thread (`kafkaTemplate.send().get()` with no timeout, `@Retryable`); no outbox (DEC-001 deviation) | COMP-05 NAPDisputeEventProcessor — consumer-side source-verified; inbound key read into local variable `keyMerchantId` via `@Header(KafkaHeaders.RECEIVED_KEY)`, logged only, not used for routing | **Per-endpoint** — `merchantId` (POST `/{caseId}/{uniqueId}` case update; POST `/outcome/...` win/loss); `cardAcceptorCodeId` (POST `/event` new disputes only — sourced from `transactions[0].meta.card_acceptor_code_id`) — partial DEC-003 deviation. See note [2]. | ✅ Production | `NapEvent` object, ≈138 fields. See WDP-COMP-04-NAP-DISPUTE-EVENT-SERVICE.md Block C for population mechanism, enriched / pass-through / service-set classification, and the known PCI concern (CVV in `toString`). |
...
| `business-rules` (prod) / `business-rules-cert` / `business-rules-dev` | COMP-12 InboundDisputeEventScheduler (Scheduler4, component=BUSINESS_RULES rows); COMP-15 EvidenceConsumer (WDP path, isMultiDocPending=false — DEC-001 deviation, inside `@Transactional` via `kafkaTemplate.send(...).get()`); COMP-23 CaseManagementService (after every material case write — DEC-001 deviation, kafka-before-commit pattern); COMP-24 CaseActionService (after action create/update — DEC-001 PARTIAL — BRE inside @Transactional); COMP-25 NotesService (non-SNOTE writes only — DEC-001 deviation; payload is the 11-field `AddNotesBREvent` structure — `eventType="NOTE_ADDED"`, `startRuleGroup="NOTE_ADDED_TO_CASE"`, `previousActionSequence` and `documentNameList` always null); COMP-37 DocumentManagementService (caseNumber key — DEC-003 deviation — publishes on upload Endpoints 1, 9, 10 and questionnaire Endpoint 11, gated by `notifyBRQueue` + `startRuleGroup`; Endpoint 8 NAP base64 path never publishes; questionnaire path wraps publish in `@Transactional(rollbackOn=Exception.class)` — stronger atomicity than other publishers on this topic) ✅ COMP-14 candidacy resolved — **CaseCreationConsumer is NOT a publisher** (no `KafkaTemplate`, no `ProducerFactory`, no reference to `business-rules` topic in source). | COMP-16 BusinessRulesProcessor | `caseNumber` — ⚠️ DEC-003 deviation, not merchantId (uniform across all six publishers) | ✅ Production | See WDP-COMP-16-BUSINESS-RULES-PROCESSOR.md Block B |
| `outgoing-events` (prod) / `outgoing-events-{dev/uat/stg/cert}` | COMP-16 BusinessRulesProcessor | COMP-18 NotificationOrchestrator | `caseNumber` — ⚠️ DEC-003 deviation. Published in `finally` block — always emitted if `caseDetails + currentActionDetails` non-null. ⚠️ DEC-001 **PARTIAL (v1.3, 2026-05-15)** — `WDP.bre_orchestration_outbox` now tracks per-event status, but the Kafka publish remains direct synchronous on the listener path (no relay between business processing and the publish call). See DEC-001 deviation row for COMP-16. | ✅ Production | See WDP-COMP-16-BUSINESS-RULES-PROCESSOR.md Block C |
| `internal-integration-events` | COMP-19 AcceptService (caseNumber key — ⚠️ DEC-003 deviation); COMP-20 ContestService (merchantId key — DEC-003 compliant; up to **2** publishes per request when CRMR action code present — branches are `if/else if`, mutually exclusive); COMP-16 BusinessRulesProcessor (conditional — UK/NAP path only, caseNumber key — ⚠️ DEC-003 deviation — DMT001-only publish is independent of the finally-block `outgoing-events` publish; both can fire for the same message) | COMP-39 NAPOutcomeProcessor (NAP platform filter only — silently discards non-NAP); COMP-40 VisaResponseQuestionnaire (non-null visaResponseIds filter only — silently discards AcceptService events) | Varies by publisher — caseNumber (COMP-19, COMP-16) or merchantId (COMP-20) | ✅ Production | See WDP-COMP-19-ACCEPT-SERVICE.md + WDP-COMP-20-CONTEST-SERVICE.md Block C |
...
[1] **business-rules topic publishers — finalised list:** Six confirmed publishers — COMP-12 (Scheduler4 bre_orchestration_outbox rows), COMP-15 (WDP path, isMultiDocPending=false), COMP-23, COMP-24, COMP-25, COMP-37 (upload Endpoints 1/9/10 and questionnaire Endpoint 11). All use `caseNumber` as partition key — DEC-003 uniform deviation. **COMP-14 candidacy is resolved — NOT a publisher** (confirmed by absence audit: zero `KafkaTemplate`, zero `ProducerFactory`, zero references to `business-rules` topic in `gcp-case-creation-consumer` source). This closes Observability-doc OQ-02. ⚠️ **COMP-22 DisputeService — Kafka producer wired to `business-rules` but call site commented out** — effectively Kafka-free at runtime. Disablement attribution: commit `c29018cd` by Shringi Nitin (WP) on 2025-08-08, message "code changes (#93)". No in-source rationale recorded.

[2] **`nap-dispute-events` partition key deviation from DEC-003:** New dispute events (POST `/event`) use `cardAcceptorCodeId` (sourced from `transactions[0].meta.card_acceptor_code_id`). Update path (POST `/{caseId}/{uniqueId}`) and outcome path (POST `/outcome/...`) use `merchantId` (compliant). All other event types correctly use `merchantId`. ⚠️ **Decommission-scoped** — this topic will be retired when the NAP/WPG inbound path is decommissioned. See WDP-COMP-04, Key Architectural Decisions.

[3] **AcceptEvent split-brain on `internal-integration-events`:** AcceptEvent can be published in two scenarios where no card-network notification actually occurred: (a) MC CHI on NAP, (b) AMEX/DISCOVER on NAP with eligible inbound actionCode. NAPOutcomeProcessor consumers should NOT assume that an AcceptEvent implies the network was successfully notified. ADR pending.
...
| `business-rules-processor-group` (prod) / `business-rules-group` (dev/local) | `business-rules` | COMP-16 BusinessRulesProcessor | **Two-branch pre-ACK (v1.3, 2026-05-15).** `MANUAL_IMMEDIATE` on both branches; branch selected on `eventId` null vs non-null. **Branch A (eventId non-null):** ACK precedes `processRulesEvent`. **Branch B (eventId null):** `processNewCaseActionEvent` persists `WDP.bre_orchestration_outbox` BUSINESS_RULES row first (carrying `idempotencyId + eventTimestamp` dedup), then ACK, then `processRulesEvent`. Both branches at-most-once. Concurrency: 1 (Spring default — `setConcurrency()` not invoked). `auto.offset.reset=latest`, `enable.auto.commit=false`, `allow.auto.create.topics=false`. `${max_poll_records}` / `${session_timeout_ms}` / `${heartbeat_interval_ms}` env-injected. Empty anonymous `CommonErrorHandler{}` — silent drop. SASL_SSL + AWS_MSK_IAM. Inbound headers: `RECEIVED_KEY → keyMerchantId` (logged only); `idempotency-key → idempotencyId` (now used for Branch B dedup against outbox; outbound passthrough unchanged). | ✅ Confirmed from COMP-16 v1.3 source |
...
| `case-action-events-group` (prod) / `case-action-events-group-cert` (cert) | `case-action-events` | COMP-17 CaseExpiryUpdateConsumer | `MANUAL_IMMEDIATE` + `syncCommits=true` — ⚠️ ACK timing inconsistent: Path A pre-ACK before all writes; Path B mid-flow ACK after outbox INSERT but before `case_expiry` write; header-blank path post-error-row INSERT. DEC-005 deviation. Concurrency: 1 (Spring default). **Max poll records: 500**. **Max poll interval: 600 000 ms (10 min)**. `auto.offset.reset=latest`, `enable.auto.commit=false`, `allow.auto.create.topics=false`. SASL/SSL: `AWS_MSK_IAM` over `SASL_SSL`. Wire-level Kafka headers are kebab-case: `idempotency-key`, `event-timestamp`. Bad-payload behaviour: deserialisation failure → NPE at path-selection branch, **no ACK in either path**; same poison message redelivers indefinitely until `max.poll.interval.ms` (10 min) trips a rebalance. | ✅ Confirmed from COMP-17 source |
| `${kafka_consumer_topic_groupId}` — env-injected via `${spring.kafka.consumer.groupId}`; value not in source | `internal-integration-events` *(confirmed runtime value — was previously inferred; resolves via `application.yml` from env var `${kafka_consumer_topic}`)* | COMP-39 NAPOutcomeProcessor | Pre-ACK — `acknowledgment.acknowledge()` as first call in `onMessage()`, before all processing (DEC-005 deviation). Concurrency: 1 (Spring Kafka default — `setConcurrency()` not invoked). `${max_poll_records}` / `${max_poll_interval}` env-injected. `auto.offset.reset=latest` — cold-start backlog skipped. SASL_SSL + AWS_MSK_IAM. Filters by `platform` field in payload (NAP only). | ✅ Pattern + AckMode confirmed from COMP-39 source; group ID value TBC |
| `internal-integration-events-ques-group` | `internal-integration-events` | COMP-40 VisaResponseQuestionnaire | Pre-ACK (`MANUAL_IMMEDIATE`) — `acknowledge()` at `KafkaConsumer.java:L30` precedes `processContestEvent()` at `:L32`. At-most-once semantics on Visa-response handling. Concurrency=1. Container factory `notificationListener`. Filters by non-null `visaResponseIds` (silently discards AcceptService events). No DLQ, no local error table. Stateless. Single `@KafkaListener` in codebase. | ✅ Re-confirmed from COMP-40 source |
...
|--------------|------------|---------------------|--------------------|----|
| `wdp.chbk_outbox_row` | COMP-07 VisaDisputeBatch (`WVDPB`), COMP-08 FirstChargebackBatch (`WMFDPB`), COMP-09 CaseFillingBatch (`WMPAPB`), COMP-11 FileProcessor (`FILE_PROCESSOR`) — INSERT PENDING/SKIPPED rows; COMP-12 InboundDisputeEventScheduler (status transitions + Kafka metadata write-back on `new-case-events` and `case-evidence-events` paths only); COMP-14, COMP-15, COMP-23, COMP-24 (status updates) | COMP-12 InboundDisputeEventScheduler (Scheduler1 — reads PENDING rows) | `new-case-events`, `case-evidence-events` | Primary inbound outbox. `idempotencyId` typed as **UUID** — cross-table inconsistency with `outgoing_event_outbox` (String). |
| `wdp.outgoing_event_outbox` | **Five writers**: COMP-17 CaseExpiryUpdateConsumer (channel_type=EXPIRY_EVENTS, `WCSEEXPC`); COMP-41 ThirdPartyNotificationConsumer (channel_type=GP_EVENTS, `WNEC`); **COMP-42 BENConsumer (channel_type=BEN_EVENTS, `WBENC`)**; COMP-43 CoreNotificationConsumer (channel_type=CORE_EVENTS, `PCSECRTC`); **COMP-51 CaseExpiryProcessor (channel_type=EXPIRY_BATCH, `WCSEEXPB`; status=ERROR direct on retry exhaustion, no PUBLISHED→FAILED transition)**. ⚠️ **COMP-18 REMOVED** — was incorrectly documented as a writer; source grep confirmed zero references in `wdp-outgoing-consumer`. | COMP-12 InboundDisputeEventScheduler (Scheduler3 — reads FAILED and PENDING_DEFERRED rows only) | `case-action-events`, `core-request-events`, `external-request-events` (via `channelTypeTopicMap`) | ⚠️ Scheduler3 queries FAILED/PENDING_DEFERRED only — not PENDING, not PUBLISHED. PUBLISHED-orphan paths in COMP-41 (3 distinct paths) and COMP-43 are not auto-recovered by Scheduler3. ⚠️ **EXPIRY_BATCH rows have no identified consumer** — COMP-51 writes status=ERROR direct, Scheduler3 reads FAILED/PENDING_DEFERRED only. Architect decision required. `idempotencyId` typed as **String** — cross-table inconsistency with `chbk_outbox_row` (UUID). Scheduler3 channel_type filter behaviour is OQ-COMP41-1. |
| `wdp.bre_orchestration_outbox` | **Three writers (2026-05-15):** **COMP-16 BusinessRulesProcessor (component=BUSINESS_RULES rows — Kafka consumer path, v1.3; persists per-event status: PUBLISHED, SUCCESS, FAILED, ERROR, PENDING_DEFERRED, SKIPPED; cross-datasource non-atomic UK write window — see RISK-203);** COMP-18 NotificationOrchestrator (component=NOTIFICATION_ORCHESTRATOR rows, `WDPOUCU` — four distinct write points, no `@Transactional`); COMP-12 InboundDisputeEventScheduler (component=BUSINESS_RULES rows from outbox-relay retry path) | COMP-12 InboundDisputeEventScheduler (Scheduler4 — reads FAILED and PENDING_DEFERRED rows only) | `business-rules` (component=BUSINESS_RULES rows), `outgoing-events` *(refer COMP-18 publish targets)* | ⚠️ Scheduler4 queries FAILED/PENDING_DEFERRED only — not PUBLISHED. PUBLISHED-status orphan rows have no automatic re-drive path — **manual runbook required for RISK-015** (now extended to COMP-16 as third writer). ⚠️ **Schema name uppercase `WDP`** while sibling tables (`wdp.CASE`, `wdp.ACTION`) are lowercase `wdp` — entity annotation inconsistency, DBA confirmation pending. ⚠️ No `@UniqueConstraint` on `idempotency_id` confirmed in any of three repos — race-window when concurrent writers (e.g. COMP-16 + COMP-12 retry path) hit the same `(idempotencyId, component)` pair. |
...
| `wdp.case_expiry` | COMP-17 CaseExpiryUpdateConsumer; **COMP-51 CaseExpiryProcessor** | COMP-51 CaseExpiryProcessor (primary reader — confirmed). Other downstream readers (if any) still need cross-component sweep. | N/A — schedule tracking, not Kafka relay | ⚠️ **Shared write between COMP-17 and COMP-51 with no coordination mechanism** — 🔴 HIGH (see WDP-DB.md Section 4). UPSERT is application-level (no DB-level INSERT … ON CONFLICT). |
...
No Kafka producer, consumer, or dependency of any kind (or — in the
"staged-but-unwired" subset — Kafka libraries on the classpath but
zero producer / consumer code anywhere in source).
...
| COMP-02 UAMS | No Kafka dependency in pom.xml. No KafkaTemplate or @KafkaListener. Vestigial `KAFKA_SERVICE_ERROR` enum exists but is application-layer only — no Kafka coupling. ✅ Re-confirmed. |
| COMP-03 CHAS | No spring-kafka dependency in pom.xml. Confirmed from source. |
| COMP-06 NAPDisputeDeclineBatch | No Kafka involvement at runtime. ⚠️ **Staged-but-unwired** — Kafka client libraries (`spring-kafka`, `kafka-clients`, `aws-msk-iam-auth`) are present on the classpath as compile-scope dependencies but unused — vestigial. Build-hygiene candidate, not a Kafka registry item. ✅ Confirmed source-verified. |
| COMP-07 VisaDisputeBatch | Kafka dependencies staged (POM only) but zero producers or consumers wired. Confirmed Kafka-free — grep against source for `@EnableKafka`, `KafkaTemplate`, `ProducerFactory`, `@KafkaListener` returns zero hits. |
| COMP-08 FirstChargebackBatch | Kafka dependencies staged (`spring-kafka`, `aws-msk-iam-auth`) but zero producers or consumers wired. Confirmed Kafka-free. |
| COMP-09 CaseFillingBatch | Consistent with COMP-07/08 pattern — zero producers or consumers wired. Confirmed Kafka-free. |
| COMP-11 FileProcessor | SQS-triggered — uses Spring Cloud AWS SQS listener, not Kafka. Zero `spring-kafka`, `kafka-clients`, `spring-cloud-stream` in pom.xml. Confirmed Kafka-free. |
| COMP-13 FileAcknowledgementProcessor | Confirmed Kafka-free — zero `spring-kafka`, `kafka-clients`, `aws-msk-iam-auth` in `pom.xml` and zero `@KafkaListener` / `KafkaTemplate` in source. |
| COMP-14 CaseCreationConsumer | Consumer of `new-case-events` only. Confirmed to have **no Kafka producer side** — no `KafkaTemplate`, no `ProducerFactory`, no reference to `business-rules` topic. (Closes Observability-doc OQ-02.) |
| COMP-21 ChargebackService | REST API only. No Kafka dependency in pom.xml. Confirmed Kafka-free by full-tree grep — zero `@KafkaListener`, `KafkaTemplate`, `@EnableKafka`, `kafka.bootstrap` occurrences. |
| COMP-22 DisputeService | Kafka producer wired to `business-rules` but call site commented out — effectively Kafka-free at runtime. Disablement attribution: commit `c29018cd` by Shringi Nitin (WP) on 2025-08-08, message "code changes (#93)". No in-source rationale recorded. ✅ Re-confirmed source-verified. |
| COMP-27 CaseSearchService | REST API only. No Kafka dependency. Confirmed Kafka-free by full-tree grep — zero `@KafkaListener`, `KafkaTemplate`, `@EnableKafka`, no `spring-kafka` in pom.xml. |
...
| COMP-51 CaseExpiryProcessor | ⚠️ **Staged-but-unwired** — Spring Batch component declares `spring-kafka`, `kafka-clients`, and `aws-msk-iam-auth` as POM dependencies but no producer or consumer code exists. ✅ Confirmed source-verified. |

> 🔴 **REMOVED from this list: COMP-37 DocumentManagementService.** Previously documented as Kafka-free; source-verification has confirmed it IS a publisher of `business-rules` (caseNumber key, DEC-003 deviation) on Endpoints 1, 9, 10, and 11. See Section 3 Topic Registry.
