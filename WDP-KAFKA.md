...
### Hidden coupling — retry config (COMP-04, added 2026-04-29)
...
|-----------|-------|-----------------|
| COMP-05 NAPDisputeEventProcessor | `nap-dispute-events` | Pre-ACK — first call in `onMessage()`, before the case service invocation. ✅ Source-verified 2026-04-29. |
...
| COMP-39 NAPOutcomeProcessor | `internal-integration-events` | Pre-ACK — `acknowledgment.acknowledge()` as first call in `onMessage()`. ✅ Re-confirmed 2026-04-29. Concurrency=1 (Spring default — `setConcurrency()` not invoked). |
| COMP-40 VisaResponseQuestionnaire | `internal-integration-events` | Pre-ACK — `acknowledge()` at `KafkaConsumer.java:L30` precedes `processContestEvent()` at `:L32`. ✅ Re-confirmed 2026-04-29. |
...
| COMP-42 BENConsumer | `external-request-events` | Pre-ACK after idempotency DB check (Step 3); before CMS call (Step 6), BEN Product call (Step 7), CAS call (Step 10), BEN Kafka publish (Step 12), outbox status update (Step 13). ✅ Confirmed from COMP-42 source 2026-04-29. |
...
| Component | Topic | Deviation detail |
|-----------|-------|-----------------|
| COMP-04 NAPDisputeEventService | `nap-dispute-events` | Direct synchronous publish on the inbound HTTP thread — `kafkaTemplate.send().get()` with no timeout, `@Retryable` retry. No outbox. No at-least-once delivery guarantee. Events lost between enrichment completion and broker ACK are unrecoverable. ✅ Source-verified 2026-04-29. |
...
| COMP-25 NotesService | `business-rules` | Synchronous publish inside `@Transactional` boundary — not atomically coupled. ⚠️ **Mid-batch atomicity note (2026-04-28):** the per-event loop runs inside `@Transactional`, so partial-batch failure produces deterministic Kafka-orphan events — the failure mode is not just JVM-crash but normal partial-batch failure. |
...
| COMP-42 BENConsumer | `external-request-events` (consumer side) | Inbound key mapped to local variable `caseNumber` of type String via `@Header(KafkaHeaders.RECEIVED_KEY)` — strongly implies key is `caseNumber`, not `merchantId`. **Requires confirmation from COMP-18 producer source.** | Consumer-side observation re-confirmed 2026-04-29 |
...
| `wdp.case_expiry` | COMP-17 CaseExpiryUpdateConsumer; **COMP-51 CaseExpiryProcessor** (added 2026-04-25 — UPDATE i_retry_count, DELETE on success/CLOSED-action/retry-exhaustion) | COMP-51 CaseExpiryProcessor (primary reader — confirmed 2026-04-25). Other downstream readers (if any) still need cross-component sweep. | N/A — schedule tracking, not Kafka relay | ⚠️ **Shared write between COMP-17 and COMP-51 with no coordination mechanism** — 🔴 HIGH (see WDP-DB.md Section 4). UPSERT is application-level (no DB-level INSERT … ON CONFLICT). |
...
| COMP-02 UAMS | No Kafka dependency in pom.xml. No KafkaTemplate or @KafkaListener. Vestigial `KAFKA_SERVICE_ERROR` enum exists but is application-layer only — no Kafka coupling. ✅ Re-confirmed 2026-04-29. |
| COMP-03 CHAS | No spring-kafka dependency in pom.xml. Confirmed from source. |
| COMP-06 NAPDisputeDeclineBatch | No Kafka involvement at runtime. ⚠️ **Staged-but-unwired** — Kafka client libraries (`spring-kafka`, `kafka-clients`, `aws-msk-iam-auth`) are present on the classpath as compile-scope dependencies but unused — vestigial. Build-hygiene candidate, not a Kafka registry item. ✅ Confirmed source-verified 2026-04-30. |
...
| COMP-22 DisputeService | Kafka producer wired to `business-rules` but call site commented out — effectively Kafka-free at runtime. Disablement attribution: commit `c29018cd` by Shringi Nitin (WP) on 2025-08-08, message "code changes (#93)". No in-source rationale recorded. ✅ Re-confirmed source-verified 2026-04-28. |
...
| COMP-51 CaseExpiryProcessor | ⚠️ **Staged-but-unwired** — Spring Batch component declares `spring-kafka`, `kafka-clients`, and `aws-msk-iam-auth` as POM dependencies but no producer or consumer code exists. ✅ Confirmed source-verified 2026-04-25. |
...
*Version 2.2 — reconciled from WDP-CHANGE-LOG.md Pending Entries for 18 components*
